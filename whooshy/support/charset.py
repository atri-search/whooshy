...
See :class:`whooshy.analysis.CharsetTokenizer` and :class:`whooshy.analysis.CharsetFilter`.
"""
...
accent_map = dict((ord(k), v) for k, v in accent_map.items())
...
def charspec_to_int(string: str) -> int:
...
def charset_table_to_dict(tablestring: str) -> Dict[int, chr]:
    ...
    map: DefaultDict[int, chr] = defaultdict(lambda: None)
    for line in tablestring.split("\n"):
        ...
        for item in line.split(","):
            ...
            if match:
                ...
                try:
                    for fromord, tooord in zip(range(start1, end1 + 1),
                                               range(start2, end2 + 1)):
